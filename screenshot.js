...
sections.forEach(section => {
  ...
  const filePath = `screenshots/${section.name}.png`;
  ...
  try {
    execSync(
      `npx capture-website-cli http://localhost:3001/#${section.name} --output screenshots/${section.name}.png ${section.fullPage ? '--full-page' : ''} --width 1920 --height 1080 --delay 2`,
      { stdio: 'inherit' }
    );
    console.log(`Screenshot saved to screenshots/${section.name}.png`);
  } catch (error) {
    console.error(`Failed to capture ${section.name}:`, error.message);
  }
});

console.log('All screenshots captured! Check the screenshots directory.'); 